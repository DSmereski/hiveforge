"""Terry adapter — text half of Terry (voice is a separate WS route)."""
...
def _terry_base_prompt(catalog: ImageCatalog | None) -> str:
    """Terry's base = LLMClient default + a short LoRA-catalog pointer.
...
class TerryAdapter:
    ...
    # `name` stays "terry" for /v1/chat/terry route compat + history
    ...
    def __init__(
        ...
        base = _terry_base_prompt(image_catalog)
    ...
    def reload_image_catalog(self, catalog: ImageCatalog) -> None:
        ...
        base = _terry_base_prompt(catalog)
    ...
    async def reply(self, user_id: int, text: str, *, extra_system: str = "") -> str:
        ...
        The chat route uses this for Terry so it can scan for `[ASK_USER]` /
        ...
        back into history. Persisting raw markers caused Terry to see her
        own previous control codes and treat them as conversational content,
        leading to confused short replies on subsequent turns.
        """
        from gateway.conversation_markers import sanitize_terry_reply
    ...
    async def reply_stream(self, user_id: int, text: str) -> AsyncIterator[str]:
        ...
        route uses .reply() for Terry now to enable marker scanning."""
        loop = asyncio.get_running_loop()
