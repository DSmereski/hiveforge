...
def test_record_and_recent_returns_for_device():
    ...
    store.record(device_id="d1", bot="terry", job_id="j1", prompt="cat")
...
def test_recent_isolates_devices():
    store = RecentImagesStore()
    store.record(device_id="d1", bot="terry", job_id="j1", prompt="cat")
    store.record(device_id="d2", bot="terry", job_id="j2", prompt="dog")
...
def test_recent_filters_by_since_ts():
    store = RecentImagesStore()
    store.record(device_id="d1", bot="terry", job_id="old", prompt="x")
    time.sleep(0.01)
    cutoff = time.time()
    time.sleep(0.01)
    store.record(device_id="d1", bot="terry", job_id="new", prompt="y")
...
def test_recent_filters_by_bot():
    store = RecentImagesStore()
    store.record(device_id="d1", bot="terry", job_id="t1", prompt="x")
    store.record(device_id="d1", bot="maggy", job_id="m1", prompt="y")
    out = store.recent(device_id="d1", bot="terry")
    assert [j.job_id for j in out] == ["t1"]


def test_recent_returns_newest_first():
    store = RecentImagesStore()
    store.record(device_id="d1", bot="terry", job_id="a", prompt="x")
    time.sleep(0.01)
    store.record(device_id="d1", bot="terry", job_id="b", prompt="y")
    time.sleep(0.01)
    store.record(device_id="d1", bot="terry", job_id="c", prompt="z")
    out = store.recent(device_id="d1")
...
def test_per_device_cap_evicts_oldest():
    ...
    for i in range(5):
        store.record(device_id="d1", bot="terry", job_id=f"j{i}", prompt=str(i))
    out = store.recent(device_id="d1")
